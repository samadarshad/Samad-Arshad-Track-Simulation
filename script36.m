...
Collisions.on = 0; %turn on to activate collision detection and default automatic overtaking procedures
...
%0
visual_speed = 0.1; %speed up or slow down simulation 
...
riders(2).lane = 3*ones(1,race_length);%[lane_ramp 2*ones(1,race_length-size(lane_ramp,1))];
...
%             %temporary
    ...
    if ~Collisions.RedoTimeStep || ~Collisions.on
        ...
        AeroGrid.Coeffs = energy_sum_aerogrid(AeroGrid.Coeffs);
