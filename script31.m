...
dt =0.02; %seconds - arbitrary - used to update velocity per acceleration time step
num_players =1;
...
riders(1).power_input = power1(3295:race_length+3295-1);
riders(1).color = 'ro';
...
riders(1).lane = 1*ones(1,race_length);
...
if plot_incline_pwr
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_incline_pwr) = plot(time_var,riders(player).power_inclination,'Color',riders(player).property_color);
    end
    xlim([time_var(1),time_var(scroll_length)])
end
...
if plot_drag_c
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_drag_c) = plot(time_var,riders(player).Cd_A,'Color',riders(player).property_color);
