...
riders(1).position(1,:) = [-25 0 0];
%riders(1).power_input = test_power*ones(1,race_length);
riders(1).power_input = power1(3295:race_length+3295);
riders(1).color = 'ro';
...
for pl = 1:num_players
    ...
    %physiological
    riders(pl).Pmax = 976; %maximum power initially, watts
    riders(pl).constrained_Pmax = 976*ones(1,race_length); %this is the constrained maximum power that decreases according to the MM model (to account for fatigue), watts
...
if plot_constrained_maxP
    ...
    hold on
    title('Maximum Power after Fatigue (Watts)','HandleVisibility','off'); %retaining title after overwrites
    set(gca,'NextPlot','replacechildren') ;
    plot_num = plot_num+3;
...
if plot_AWC
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_AWC) = plot(riders(player).AWC,'Color',riders(player).property_color);
...
if plot_constrained_maxP
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_constrained_maxP) = plot(riders(player).constrained_Pmax,'Color',riders(player).property_color);
...
if plot_AL
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_AL) = plot(riders(player).AL,'Color',riders(player).property_color);
...
if plot_L
    ...
    for player = 1:num_players
        ...
        riders(player).gobjects(plot_L) = plot(riders(player).L,'Color',riders(player).property_color);
