...
if new
    load('Working14feb.mat', 'base', 'sm_lane', 'sm_red_lane', 'sm_blue_lane', 'MeshSt')
    clearvars lanes
...
Aero_fig = subplot(3,3,[3 6 9],ax1);
%view(2)
...
for i= 1:race_length
    ...
    for player = 1:num_players
        ...
        riders(player).Cd_A(i) = riders(player).base_Cd_A*get_aero_coeff2(AeroGrid,riders(player).position(i-1,:),riders(player).position(i,:));
        end
