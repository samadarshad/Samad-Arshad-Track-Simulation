...
riders(i).lane_idx =  dsearchn([lanes(lane_number).X lanes(lane_number).Y],riders(i).position);
riders(i).position = [lanes(lane_number).X(riders(i).lane_idx) ...
                      lanes(lane_number).Y(riders(i).lane_idx) ...
                      lanes(lane_number).Z(riders(i).lane_idx) ];
riders(i).vectors = get_vects(MeshSt,riders(i).position);
end
...
riders(player).gobjects(4) = point_plot2(riders(player).position, riders(player).color, h);
end
...
% initial plot, height
 ...
 hold on
    ...
    riders(player).height(1) =riders(player).position(3); 
...
for player = 1:num_players    
    ...
    riders(player).position = new_pos;
    riders(player).height(i) = riders(player).position(3);
    ...
    riders(player).vectors = get_vects(MeshSt,riders(player).position);
    riders(player).steer_angle(i) = get_steer(riders(player).vectors.cx,prev_dir);
    riders(player).bank_angle(i) = get_bank(riders(player).position,riders(player).vectors);
...
riders(player).gobjects(4) = update_plot(riders(player).position, riders(player).gobjects(4));
end
