...
riders(2).lane = 1*ones(1,race_length);
riders(3).lane = 3*ones(1,race_length);
riders(4).lane = 3*ones(1,race_length);
%for all players
...
%                 riders(1).lane = 3;
            if ~isempty(Collisions.OverTakersID) && ismember(Collisions.OverTakersID,player) %LANE CHANGE due to collision detection
                new_lane = riders(player).lane(i) + 1;
                [new_idx, new_pos] = step_lane_w_change(riders(player).lane_idx,riders(player).v_w(i),riders(player).lane(i),new_lane,lanes);
                 riders(player).lane(i) = new_lane;
                 prev_dir = prev_dir;
%                  Collisions.OverTakersID = [];
%                  Collisions.BeingOverTakenID  = [];
                Collisions.OverTakersID(find(Collisions.OverTakersID==player))=[];
            elseif ~isempty(Collisions.OverTakersID) && ~ismember(Collisions.OverTakersID,player)
                ...
                Collisions.OverTakersID(find(Collisions.OverTakersID==player))=[];
            elseif isempty(Collisions.OverTakersID)
                [new_idx, new_pos] = step_lane(riders(player).lane_idx,riders(player).v_w(i),lanes(riders(player).lane(i)));
                prev_dir = riders(player).vectors.cx; %temporary for 1-time step for steering angle. Could incoorporate smoothening over more time steps
            else
                new_idx = new_idx;
...
%
    ...
    if isempty(Collisions.OverTakersID)
